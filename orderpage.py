...
def insert():
    ...
    if not e1.get()or not e2.get()or not e3.get() or not e4.get() or not e5.get()or not e6.get():
        messagebox.showinfo("","please enter value")
    
    else:
        if not oid.isdigit()or cname.isdigit()or not pprice.isdigit()or pbrand.isdigit()or not cmobile.isdigit()or pname.isdigit():
            ...
            data=(oid,cname,pprice,pbrand,cmobile,pname)
...
def update():
    ...
    if not e1.get()or not e2.get()or not e3.get() or not e4.get() or not e5.get()or not e6.get():
        ...
        if not oid.isdigit() or cname.isdigit()or not pprice.isdigit() or pbrand.isdigit() or not cmobile.isdigit()or pname.isdigit():
            ...
            mydb=mysql.connector.connect(host='localhost',user='root',password='',database='project')
            mycursor=mydb.cursor()
            sql ="update demo set cname=%s,pprice=%s,pbrand=%s,cmobile=%s,pname=%s where oid=%s"
            data =(cname,pprice,pbrand,cmobile,pname,oid)
            mycursor.execute(sql,data)
            mydb.commit()
...
def delete():
    ...
    if not e1.get():
        ...
        if not oid.isdigit():
            ...
            mydb=mysql.connector.connect(host='localhost',user='root',password='',database='project')
            mycursor=mydb.cursor()
            sql="delete from demo where oid='%s'"
...
l2=Label(w,text="Order ID:",font=10)
l2.place(x=40,y=50)
...
l7=Label(w,text="Product Name:",font=10)
l7.place(x=40,y=300)
